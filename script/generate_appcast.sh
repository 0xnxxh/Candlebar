...
ACCOUNT="${SPARKLE_KEY_ACCOUNT:-0xnxxh.Candlebar}"
DIST_DIR="$ROOT_DIR/dist"
DMG_PATH="$DIST_DIR/Candlebar-v$APP_VERSION.dmg"
APPCAST_PATH="$DIST_DIR/appcast.xml"
APPCAST_WORK_DIR="$DIST_DIR/appcast-work"
RELEASE_BASE_URL="${RELEASE_BASE_URL:-https://github.com/0xnxxh/Candlebar/releases/download/v$APP_VERSION/}"
...
if [[ ! -f "$DMG_PATH" ]]; then
  "$ROOT_DIR/script/release_dmg.sh" >/dev/null
fi
